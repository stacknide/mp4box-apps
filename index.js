...
function addBuffer(video, mp4track) {
	...
	if (MediaSource.isTypeSupported(mime)) {
		try {
			...
			sb.ms = ms;
			sb.id = track_id;
			mp4boxfile.setSegmentOptions(track_id, sb, { nbSamples: parseInt(segmentSizeLabel.value) } ); // TODO:
			sb.pendingAppends = [];
		} catch (e) {
		...
		if ((!foundTextTrack) && (html5TrackKind !== "")) {
			...
			overlayTracks.appendChild(div); // TODO
			texttrack.div = div;
...
function addSourceBufferListener(info) {
	for (var i = 0; i < info.tracks.length; i++) {
		...
		checkBox.addEventListener("change", (function (t) { // TODO:
			return function (e) {
				var check = e.target;
				if (check.checked) { 
					addBuffer(video, t);
...
function initializeSourceBuffers() {
	...
	initAllButton.disabled = true; // TODO	
...
function initTrackViewer(track) { // TODO:
...
function load() {
	...
	mp4boxfile.onReady = function (info) {
		...
		if (autoplay) {
			...
			initAllButton.disabled = false; // TODO: removed
		}
	}
	...
	downloader.setCallback(
		function (response, end, error) { 
			var nextStart = 0;
			if (response) {
				progressbar.progressbar({ value: Math.ceil(100*downloader.chunkStart/downloader.totalLength) }); // TODO:
	...
	downloader.setInterval(parseInt(chunkTimeoutLabel.value)); // TODO:
...
function start() {
	...
	downloader.setChunkSize(parseInt(chunkSizeLabel.value)); // TODO
...
function stop() {
	if (!downloader.isStopped()) {
		stopButton.disabled = true; // TODO:
		startButton.disabled = false; // TODO:
		downloader.stop();
	}
}		
