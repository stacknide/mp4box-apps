 ...
 * @constructor
 * @param {ArrayBuffer|DataView|number} arrayBuffer ArrayBuffer to read from.
 ...
 */
MultiBufferStream.prototype.reduceBuffer = function (
  buffer,
  offset,
  newLength
) {
...
var BoxParser = {
  ...
  createMediaSampleEntryCtor: function (mediaType, parseMethod, subBoxNames) {
    ...
    BoxParser[mediaType + "SampleEntry"].prototype =
  ...
  createSampleGroupCtor: function (type, parseMethod) {
    ...
    BoxParser[type + "SampleGroupEntry"].prototype =
      new BoxParser.SampleGroupEntry();
    if (parseMethod)
  ...
  createTrackGroupCtor: function (type, parseMethod) {
    ...
    BoxParser[type + "TrackGroupTypeBox"].prototype =
...
BoxParser.createFullBoxCtor("auxC", function (stream) {
  ...
  var aux_subtype_length =
    this.size - this.hdr_size - (this.aux_type.length + 1);
...
/** file:src/parsing/singleitemtypereference.js // TODO: add proper JSDocs with constructor
 * @type {Class} BoxParser.SingleItemTypeReferenceBox
 */
...
/** file:src/parsing/singleitemtypereferencelarge.js // TODO: add proper JSDocs with constructor
 * @type {Class} BoxParser.SingleItemTypeReferenceBoxLarge
 */
...
BoxParser.createFullBoxCtor("tfra", function (stream) {
  ...
  for (var i = 0; i < number_of_entries; i++) {
    ...
    this.traf_number =
      stream["readUint" + 8 * (this.length_size_of_traf_num + 1)]();
    this.trun_number =
      stream["readUint" + 8 * (this.length_size_of_trun_num + 1)]();
    this.sample_number =
...
/** // TODO: add proper JSDocs with constructor
 * @type {Class} BoxParser.TrackReferenceTypeBox
 */
...
BoxParser.createFullBoxCtor("vvcC", function (stream) {
  ...
  var bitReader = {
    ...
    extract_bits: function (num_bits) {
      var ret =
        (this.held_bits >> (this.num_held_bits - num_bits)) &
        ((1 << num_bits) - 1);
  ...
  for (i = 0; i < num_of_arrays; i++) {
    ...
    var numNalus = 1;
    if (
      nalu_array.nalu_type != VVC_NALU_DEC_PARAM &&
      nalu_array.nalu_type != VVC_NALU_OPI
    ) {
 ...
 * @type {Class} ISOFile
 * @constructor
 * @param {MultiBufferStream} stream
 * @returns {ISOFile}
 */
...
ISOFile.prototype.initialize = function (stream) {
...
ISOFile.prototype.createSingleSampleMoof = function (sample) {
  ...
  traf
    .add("tfdt")
    .set("baseMediaDecodeTime", sample.dts - (trak.first_dts || 0));
  traf
    .add("trun")
    .set(
      "flags",
...
ISOFile.initSampleGroups = function (
  trak,
  traf,
  sbgps,
  trak_sgpds,
