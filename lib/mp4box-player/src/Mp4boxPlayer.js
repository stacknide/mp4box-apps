...
export class Mp4boxPlayer {
  constructor(videoElement, initialDownloadConfig, downloaderInstance) {
    ...
    this.SEGMENT_DURATION_SECS = 3; // 3sec default chunk duration, can be reduced to 2 - 2.5sec if we want a faster play/seek after testing.
    ...
    if (MediaSource.isTypeSupported(mime)) {
      try {
        ...
        this.mp4boxfile.setSegmentOptions(track_id, sb, {
          nbSamples: this.videoFps * this.SEGMENT_DURATION_SECS  //Consider changing to this.SEGMENT_DURATION_SECS/2  if prod needs more latency improvements later.
        }); // TODO:
        sb.pendingAppends = [];
      } catch (e) {
        const errId = `MSE SourceBuffer #${track_id}`;
