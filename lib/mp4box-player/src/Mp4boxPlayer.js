...
export class Mp4boxPlayer {
  constructor(videoElement, initialDownloadConfig, downloaderInstance) {
    ...
    this.desiredChunkSize = 409600; // Default 1MB chunksize
    this.CHUNK_DURATION = 3; //2sec default chunk duration, can be reduced to 1 - 1.5sec if we want a faster play/seek after testing.
  ...
  load = () => {
    ...
    this.mp4boxfile.onReady = (info) => {
      ...
      info.tracks.forEach( (track) => {
        if (track.type === "video") {
          const videoDurationInSeconds = track.duration / track.timescale;
          const frameRate = track.nb_samples / videoDurationInSeconds;
          this.videoFps = Math.ceil(frameRate);

          var durationInSeconds = track.duration / track.timescale;
          this.desiredChunkSize = (track.size / durationInSeconds) * this.CHUNK_DURATION;
        }
      });
    ...
    if (MediaSource.isTypeSupported(mime)) {
      try {
        ...
        this.mp4boxfile.setSegmentOptions(track_id, sb, {
          nbSamples: this.videoFps * (this.CHUNK_DURATION/2)
        }); // TODO:
        sb.pendingAppends = [];
      } catch (e) {
        const errId = `MSE SourceBuffer #${track_id}`;
