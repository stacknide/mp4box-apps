...
class Transcoder {
  ...
  load = async () => {
    const ffmpeg$1 = this.ffmpeg || new ffmpeg.FFmpeg();
    ffmpeg$1.on("log", ({ message }) => {
    ...
    try {
      ...
      const safeCoreUrl = await util.toBlobURL(coreUrl, "text/javascript");
      const safeWasmUrl = await util.toBlobURL(wasmUrl, "application/wasm");
      const safeWorkerUrl = this.cfg.enableMultiThreading
        ? await util.toBlobURL(workerUrl, "text/javascript")
        : undefined;
  ...
  transcode = async ({ input, ext, id = nanoid.nanoid(), timeout }) => {
    ...
    bufferFetcherThenable
      .then(async function ({ buffer: buf, totalLength }) {
        ...
        if (dl.transcoder) {
          ...
          const uInt8Output = //
            await dl.transcoder.transcode({ input: uInt8Arr, ext: "mkv" });
