import { DataStream, Log, 
// Textin4Parser,
// VTTin4Parser,
// XMLSubtitlein4Parser,
MP4Box, } from "@knide/mp4box";
    ...
    */
    ...
    onSeeking(e) {
        ...
        if (this.video.lastSeekTime !== this.video.currentTime) {
            ...
            Log.info("Application", "Seeking called to video time " +
                Log.getDurationString(this.video.currentTime));
            this.downloader.stop();
            this.resetCues();
            if (!this.mp4boxfile)
                return console.error("mp4boxfile not initialized");
    ...
    onSourceClose = (e) => {
        ...
        if (ms.video?.error) {
            ...
            this.onErrorCallback?.(`MSE, Source closed, video error`, ms.video?.error);
        }
        else {
    ...
    load = () => {
        ...
        this.mp4boxfile = MP4Box.createFile();
        if (!this.mp4boxfile)
            throw new Error("mp4boxfile ISOFile not created");
        ...
        */
        ...
        if (!this.config.url)
            Log.warn("No Video URL specified!");
    ...
    initializeSourceBuffers = () => {
        ...
        if (!this.mp4boxfile)
            throw new Error("Cannot initializeSourceBuffers. mp4boxfile not initialized");
        ...
        for (var i = 0; i < initSegs.length; i++) {
            ...
            sb.segmentIndex = 0;
            if (sb.ms.pendingInits !== undefined)
                sb.ms.pendingInits++;
        }
    ...
    start = () => {
        ...
        if (!this.mp4boxfile)
            throw Error("mp4boxfile not initialized");
        this.mp4boxfile.start();
        this.downloader.resume();
    };
    onUpdateEnd(sb, isNotInit, isEndOfAppend) {
        this.SHOW_LOGS &&
            console.log("onUpdateEnd", sb.id, sb.ms?.readyState, sb.updating, sb.pendingAppends?.length, sb.segmentIndex);
        if (isEndOfAppend === true) {
            ...
            if (sb.is_last) {
                try {
                    if (sb.ms?.readyState === "open" && !sb.updating)
                        sb.ms.endOfStream();
                }
                catch (e) {
        ...
        if (sb.ms?.readyState === "open" &&
            ...
            if (!obj)
                return console.error("obj is null");
    ...
    addBuffer = (video, mp4track) => {
        ...
        var ms = video.ms;
        if (!ms)
            return console.error(`Cannot addBuffer. video.ms is ${ms}`);
    ...
    generateMovieInfo = (info) => {
        ...
        const actualTime = `${info.duration}/${info.timescale} ` +
