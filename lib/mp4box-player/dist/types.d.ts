import type { Mp4boxSourceBuffer as _SB, Mp4boxVideoElement as _VE, Mp4boxMediaSource as _MS } from "@knide/mp4box";
export type TConfig = {
    /** Video URL */
    url: string;
...
callback: TOnTrackInfoCallback) => void;
...
declare const trackTypes: readonly ["VideoTracks", "AudioTracks", "SubtitleTracks", "MetadataTracks", "OtherTracks"];
...
export interface TMovieInfo {
    ...
    tableRows: {
        label: string;
        value: string | boolean | number | FmtDate;
    }[];
    trackInfo: {
        type: string;
        tracks: Mp4boxMovieInfo[TrackKey];
    }[];
...
callback: TOnMovieInfoCallback) => void;
...
/*************Downloader.js********************* */
export type TBufferFetcher = (start: number, end: number, signal: AbortSignal) => Promise<{
    buffer: ArrayBuffer;
    totalLength: number;
}>;
export type TOnDownloadCallback = (buffer: ArrayBuffer | null, end: boolean, error?: boolean | Error) => Promise<void> | void;
