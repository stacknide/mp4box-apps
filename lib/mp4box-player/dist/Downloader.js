...
export class Downloader {
    ...
    getFile = () => {
        ...
        const defaultBufferFetcher = async (start, end, signal) => {
            var range = "";
            var maxRange;
            if (end < Infinity) {
                range = "bytes=" + start + "-";
        ...
        bufferFetcherThenable
            ...
            if (dl.ffmpeg) {
                ...
                buffer = buf;
            if (totalLength)
                dl.totalLength = totalLength;
            else
                throw new Error("file byte length not available");
            ...
            if (dl.eof) {
                ...
                Log.info(`EOF:${dl.eof}`, `Received Buffer Length: ${buffer.byteLength.toLocaleString()}, Expected Length: ${dl.chunkSize.toLocaleString()}, Total Length: ${dl.totalLength.toLocaleString()}`);
    ...
    computeWaitingTimeFromBuffer(video) {
        ...
        Log.info("Demo", "Playback position (" +
