...
export class Mp4boxPlayer {
  ...
  /**
   * Sets a callback when a control's status has changed.
   *
   * @param {StatusChangeCallback} callback - Called when the status of a control needs to be changed.
   *   This function should accept parameters for controlName and status.
   *   - {string} controlName - Name of the control.
   *   - {any} status - Represents boolean for "enabled" status, number for "value" status, string for "text" status.
   */
  setOnStatusChangeCallback = (callback) => {
    this.onStatusChangeCallback = callback;
  };

  onStatusChange = (controlName, status) => {
    this.onStatusChangeCallback?.(controlName, status);
  };
  ...
  onSeeking(e) {
    ...
    if (this.video.lastSeekTime !== this.video.currentTime) {
      ...
      this.onStatusChange("start", false);
      this.onStatusChange("stop", true);
  ...
  resetDisplay = () => {
    ...
    // this.overlayTracks.innerHTML = ""; // TODO: Element
    //video.poster = '';
    //video.playing = false;
  };
  ...
  stop = () => {
    ...
    if (!this.downloader.isStopped()) {
      this.onStatusChange("stop", false);
      this.onStatusChange("start", true);
      this.downloader.stop();
    }
  };
  ...
  play = () => {
    ...
    this.onStatusChange("play", false);
    this.autoplay = true;
    this.video.play();
    this.load();
  };
  ...
  load = () => {
    ...
    this.mp4boxfile.onReady = (info) => {
      ...
      if (this.autoplay) {
        ...
        this.onStatusChange("initializeAllSourceBuffers", true);
      }
    };
    ...
    this.onStatusChange("load", false);
    this.onStatusChange("start", false);
    this.onStatusChange("stop", true);
  ...
  initializeSourceBuffers = () => {
    ...
    this.onStatusChange("initializeAllSourceBuffers", false);
    this.onStatusChange("initializeSourceBuffers", false);
  };
  /* Load END */
      ...
      checkBox.addEventListener(
        ...
        ((t) => (e) => {
          ...
          if (check.checked) {
            ...
            this.onStatusChange("initializeSourceBuffers", true);
            this.onStatusChange("initializeAllSourceBuffers", false);
          } else {
            const isDisabled = this.removeBuffer(this.video, t.id);
            this.onStatusChange("initializeSourceBuffers", !isDisabled);
            this.onStatusChange("initializeAllSourceBuffers", !isDisabled);
          }
        })(track)
      );
    }
