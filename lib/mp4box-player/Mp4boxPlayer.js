...
export class Mp4boxPlayer {
  ...
  /** Info START */
  generateMovieInfo(info, _displaySourceBuffer) {
    if (_displaySourceBuffer !== undefined)
      displaySourceBuffer = _displaySourceBuffer;

    const fileLength =
      typeof downloader !== "undefined" ? downloader.getFileLength() : 0;
      ...
      tableRows: [
        ...
        },
        {
          ...
          value:
            info.created &&
            info.modified &&
            `${dateToInput(info.created)} / ${dateToInput(info.modified)}`,
        },
      ],
      trackInfo: [],
    };
