...
export class Mp4boxPlayer {
   ...
   *
   * @param {function} callback - called when the status of a control needs to be changed
   *   This function should accept parameters for controlName and status
   *   - {string} controlName - name of the control
   *   - {boolean} status - represents "enabled" status
   */
  setOnStatusChangeCallback = (callback) => {
    this.onStatusChangeCallback = callback;
  };
  onStatusChange = (controlName, isEnabledStatus) => {
    this.onStatusChangeCallback?.(controlName, isEnabledStatus);
  };
 ...
 * @property {function} initializeAllSourceBuffers - Adds a new source buffer to the player + initializes the source buffer.
